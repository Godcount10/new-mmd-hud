...
async function start(): Promise<void> {
  ...
  if (new URLSearchParams(location.search).has('injection')) {
    const response = await fetch(`/dist-hud/${__HUD_INSTANCE__}/mmd-hud.json`)
    ...
    if (!new URLSearchParams(location.search).has('remote')) instance.prepareLocalInjection?.(window)
    ...
    csp.content = "connect-src 'none'; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; img-src 'self' data: blob: https://cdn.jsdelivr.net"
